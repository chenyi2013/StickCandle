...
public class KChartsView extends GridChart implements
		...
		if (showDetails) {
			...
			try {
				...
				canvas.drawText(new DecimalFormat("#.##").format(open), left
						+ 1 + DEFAULT_AXIS_TITLE_SIZE * 2.5f, top
	...
	private void drawTitles(Canvas canvas) {
		...
		canvas.drawText(new DecimalFormat("#.##").format(mMinPrice), 1,
				UPER_CHART_BOTTOM - 1, textPaint);
		canvas.drawText(
				new DecimalFormat("#.##").format(mMinPrice
						+ (mMaxPrice - mMinPrice) / 4), 1, UPER_CHART_BOTTOM
						- getLatitudeSpacing() - 1, textPaint);
		canvas.drawText(
				new DecimalFormat("#.##").format(mMinPrice
						+ (mMaxPrice - mMinPrice) / 4 * 2), 1,
				UPER_CHART_BOTTOM - getLatitudeSpacing() * 2 - 1, textPaint);
		canvas.drawText(
				new DecimalFormat("#.##").format(mMinPrice
						+ (mMaxPrice - mMinPrice) / 4 * 3), 1,
		...
		try {
			...
			canvas.drawText(
					String.valueOf(mOHLCData.get(
							mDataStartIndext + mShowDataNum - 1).getDate()), 2,
					UPER_CHART_BOTTOM + DEFAULT_AXIS_TITLE_SIZE, textPaint);
		} catch (Exception e) {
	...
	private void drawUpperRegion(Canvas canvas) {
		...
		Log.i("eileen", "candleWidth:" + mCandleWidth);
		...
		for (int j = 0; j < MALineData.size(); j++) {
			...
			canvas.drawText(
					lineEntity.getTitle()
							+ "="
							+ new DecimalFormat("#.##").format(lineEntity
									.getLineData().get(mDataStartIndext)), 2
